...
func generate() {
	...
	fmt.Printf("p1, _ := new(BigInt.Nat).SetHex(\"%s\")\n", sk1.P().Hex())
	fmt.Printf("q1, _ := new(BigInt.Nat).SetHex(\"%s\")\n", sk1.Q().Hex())
	fmt.Printf("p2, _ := new(BigInt.Nat).SetHex(\"%s\")\n", sk2.P().Hex())
	fmt.Printf("q2, _ := new(BigInt.Nat).SetHex(\"%s\")\n", sk2.Q().Hex())
	...
	fmt.Printf("s, _ := new(BigInt.Nat).SetHex(\"%s\")\n", ped.S().Hex())
	fmt.Printf("t, _ := new(BigInt.Nat).SetHex(\"%s\")\n", ped.T().Hex())
	fmt.Println("Pedersen, _ = pedersen.New(VerifierPaillierPublic.N(), s, t)")
}

func init() {
	p1, _ := new(BigInt.Nat).SetHex("")
	q1, _ := new(BigInt.Nat).SetHex("")
	p2, _ := new(BigInt.Nat).SetHex("")
	q2, _ := new(BigInt.Nat).SetHex("")
