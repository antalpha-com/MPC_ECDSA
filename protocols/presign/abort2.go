...
func (r *abort2) StoreBroadcastMessage(msg round.Message) error {
	...
	for id, chiProof := range body.ChiProofs {
		alphas[id] = r.Group().NewScalar().SetNat(chiProof.Plaintext.Mod1(r.Group().Order()))
	}
	r.ChiAlphas[from] = alphas
	r.YHat[from] = body.YHat
	r.KShares[from] = r.Group().NewScalar().SetNat(body.KProof.Plaintext.Mod1(r.Group().Order()))

	if !body.YHatProof.VerifyMal(r.Group(), r.HashForID(from), zklog.Public{
		H: r.ElGamalK[from].L,
		X: r.ElGamal[from],
		Y: body.YHat,
	}) {
