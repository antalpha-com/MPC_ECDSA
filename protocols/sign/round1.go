...
func (r *round1) Finalize(out chan<- *round.Message) (round.Session, error) {
	...
	//Broadcast the broadcastMsg to all other parties using the BroadcastMessage method with the out channel.
	if err := r.BroadcastMessage(out, &broadcastMsg); err != nil {
		return r, err
	}
	errors := r.Pool.Parallelize(len(otherIDs), func(i int) interface{} {
		j := otherIDs[i]
		//Generate a zero-knowledge encryption proof
		...
		//Send the proof to party j with the out channel.
